...
void usage(char* name) {
    fprintf(stderr, "USAGE: %s local_socket_name tcp_port\n", name);
    exit(EXIT_FAILURE);
}
...
int main(int argc, char** argv) {
    if(argc != 3) usage(argv[0]); // Zmiana liczby argumentów!
    ...
    // Gniazdo Lokalne
    int local_sfd = bind_local_socket(argv[1], 10);
    if(local_sfd < 0) ERR("bind_local_socket");

    // Gniazdo TCP
    uint16_t port = atoi(argv[2]);
    int tcp_sfd = bind_tcp_socket(port, 10);
    if(tcp_sfd < 0) ERR("bind_tcp_socket");
    ...
    // Dodajemy oba gniazda do epolla
    ev.events = EPOLLIN;
    ev.data.fd = local_sfd;
    if(epoll_ctl(epoll_fd, EPOLL_CTL_ADD, local_sfd, &ev)) ERR("epoll_ctl");

    ev.data.fd = tcp_sfd;
    if(epoll_ctl(epoll_fd, EPOLL_CTL_ADD, tcp_sfd, &ev)) ERR("epoll_ctl");
    ...
    while(do_work) {
        ...
        for(int i = 0; i < nfds; i++) {
            ...
            // Sprawdzamy, czy połączenie puka do gniazda lokalnego LUB sieciowego
            if(fd == local_sfd || fd == tcp_sfd) {
                int client_fd = add_new_client(fd); // Funkcja radzi sobie i z AF_UNIX i AF_INET
                if(client_fd >= 0) {
                    ev.events = EPOLLIN;
