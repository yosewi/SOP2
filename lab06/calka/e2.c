...
int main(int argc, char* argv[])
{
    ...
    if(shm_fd >= 0){
        ...
        int ret;
        ret = pthread_mutex_lock(&shared_data->mxprocCount);
        if(ret == EOWNERDEAD){
    ...
    int current_procs;
    pthread_mutex_lock(&shared_data->mxprocCount);
    ...
    for(int i = 0;i<3;i++){
        ...
        int ret = pthread_mutex_lock(&shared_data->mxResults);
        if(ret == EOWNERDEAD){
    ...
    pthread_mutex_lock(&shared_data->mxprocCount);
    shared_data->procCount--;
    int remainingprocs = shared_data->procCount;
    pthread_mutex_unlock(&shared_data->mxprocCount);
