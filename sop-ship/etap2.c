...
void msleep(int millisec)
{
    ...
    // Ochrona przed przerwaniem przez EINTR
...
int choose_card_to_discard(int *hand, int m)
{
    ...
    // POPRAWKA: AI losuje odrzucaną kartę spośród wszystkich niepasujących
    // Zapobiega to utknięciu gry w logicznej martwej pętli
    int bad_cards[52];
    int bad_count = 0;
    for (int i = 0; i < m; i++) {
    ...
    if (bad_count > 0) {
        return bad_cards[rand() % bad_count];
    }
    return rand() % m; // Jeśli wygrywa, losuje cokolwiek
}

void player_work(int deal_read_fd, int ring_read_fd, int ring_write_fd, int m)
{
    ...
    // Kuloodporny odczyt początkowych kart (ochrona przed short-read i EINTR)
    int bytes_read = 0;
    ...
    // Nieskończona pętla rozgrywki (Pierścień)
    while (1)
    {
        ...
        // 1. Wysyłanie karty z ochroną przed EINTR
        ssize_t w_ret;
        do {
        ...
        // 2. Odczyt nowej karty z ochroną przed EINTR
        ssize_t r_ret;
        do {
        ...
        if (r_ret < 0) {
            ...
            break; // Ktoś z pierścienia wyszedł i zamknął potok
        }

        msleep(200); // Zwolnienie pętli, żebyś mógł śledzić akcję w konsoli
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (pid == 0) 
        {
            // Konfiguracja indeksów potoków dla P(i) -> P(i+1)
            int read_ring_idx = i;              
            ...
            player_work(deal_read_fd, ring_read_fd, ring_write_fd, m);
    ...
    // Rodzic nie uczestniczy w ring-u, domyka swoje referencje
    for (int i = 0; i < n; i++)
    {
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        // Wysyłanie kart z ochroną przed EINTR
        ssize_t w_ret;
        do {
    ...
    // Rodzic wisi nieskończenie (zamknij skrypt przez Ctrl+C by ubić wszystko)
