...
void game_server_work(int *rps, int *wps, int n, int m){
    ...
    int played_cards[n];
    memset(played_cards, 0, sizeof(int) * n);
    int max = -1;
        ...
        if(res == -1){
            ...
            played_cards[i] = atoi(buf);
            printf("Got number %d from player %d\n", atoi(buf), i);

            if(played_cards[i] > max){
                max = played_cards[i];
            }
        }
        }
        int winners_count = 0;
        for(int i = 0;i<n;i++){
            if(rps[i] != 0 && played_cards[i] == max){
                winners_count++;
            }
        }
        int pointstogive = 0;
        if(winners_count > 0){
            pointstogive = n / winners_count;
        }
        ...
        for(int i = 0;i<n;i++){
            ...
            int pts = (played_cards[i] == max) ? pointstogive : 0;
            scores[i] += pts;

            memset(buf, 0, MSG_SIZE);
            snprintf(buf, MSG_SIZE, "%d", pts);
            
            res = write(wps[i], buf, MSG_SIZE);
            if(res == -1 && errno == EPIPE){
                close_pipe(wps[i]); wps[i] = 0;
                close_pipe(rps[i]); rps[i] = 0;
            }
            else if(res == -1) ERR("write points");
        }
    }
    printf("\n=== FINAL SCOREBOARD ===\n");
    for(int i = 0; i < n; i++){
...
void player_work(int rp, int wp, int player_idx, int m){
    ...
    for(int i = 1;i<=m;i++){
        ...
        if (rand() % 100 < RESIGN_PROBABILITY) {
            break; 
        }

        int rand_id = rand() % remaining;
        int my_card = cards[rand_id];

        cards[rand_id] = cards[remaining - 1];
        remaining--;

        memset(buf, 0, MSG_SIZE);
        snprintf(buf, MSG_SIZE, "%d", my_card);
